...
Финальный проект по машинному обучению, реализующий комплексный модульный пайплайн предобработки данных, генерации признаков (Feature Engineering), обучения ансамбля регрессионных моделей (ML + DL) и финального многослойного стекинга.
...
Целевая переменная оптимизировалась в логарифмическом масштабе (\(\log10(x + 1)\)), поэтому основной метрикой является **RMSLE** (Root Mean Squared Logarithmic Error).
...
| Averaging Ensemble | 0.1134 | Простое взвешенное усреднение |
...
│   ├── training.py        # Универсальная OOF-валидация регрессоров (run_model)
