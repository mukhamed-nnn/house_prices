...
def preprocessing(train, test, cv):
    ...
    mode_fill_columns = ['MSZoning', 'Utilities', 'Functional', 'Exterior1st',
    ...
    already_ordinal = quality_columns + ['BsmtExposure', 'BsmtFinType1', 'BsmtFinType2',
                                          'GarageFinish', 'Fence', 'PavedDrive', 'Functional']
    ...
    ohe_features, target_features = [], []
    for col in cat_columns:
        n_unique = df.loc[train_mask, col].nunique()
        (ohe_features if n_unique <= THRESHOLD else target_features).append(col)
    ...
    # cat_columns (ohe + target) — единственно верный список для CatBoost:
    # already_ordinal сюда не входит, потому что эти колонки уже числовые (порядковые).
    return train, test, train_cat, test_cat, cat_columns
